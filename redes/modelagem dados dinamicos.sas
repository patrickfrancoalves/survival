...
DATA TABELA_PETRO(COMPRESS=YES ENCODING=ANY DROP=UF_NASCIMENTO CONTINENTE SEXO
...
/*PROC FREQ DATA=TABELA_PETRO ORDER=FREQ;TABLE PETRO /NOCOL NOPERCENT NOROW;RUN;*/
/*CENPES_LATTES PETRO_LAB PETRO_LATTES PETRO_PETRO*/
...
PROC LOGISTIC DATA=TABELA_PETRO;
/*WHERE ANO>2015;*/
CLASS UF_NASCN UF_PROFN GRANDE_AREAN SEXON VINCULON LOCAL_FORMACN EXCLUSIVA;
MODEL &DEP.(REF=FIRST)=LN_IDADE LN_IDADE*t LN_COAUTOR LN_COAUTOR*t LN_CAPES LN_CAPES*t
                       UF_NASCN UF_PROFN GRANDE_AREAN SEXON VINCULON 
                       LOCAL_FORMACN EXCLUSIVA / LINK=LOGIT ;
OUTPUT OUT=SAIDA_&DEP.(KEEP=ArquivoXML CENPES_LATTES PETRO_LATTES PETRO_PETRO P) PROB=P;
RUN;
...
SELECT DISTINCT A.ArquivoXML,A.&DEP.,
       ...
       B.LN_IDADE,B.LN_COAUTOR,B.LN_CAPES,B.DIRECAO,
	   B.UF_NASCN,B.UF_PROFN,B.GRANDE_AREAN,B.SEXON,
       B.VINCULON,B.LOCAL_FORMACN,B.EXCLUSIVA,B.ANO,
	   B.LN_ARTIGOS           ,
	   B.LN_ARTIGO_BRASIL     ,
...
FROM SAIDA_&DEP. AS A
LEFT JOIN TABELA_PETRO AS B ON A.ArquivoXML=B.ArquivoXML
