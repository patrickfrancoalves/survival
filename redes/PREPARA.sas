...
*/
DATA LATTES.TABELA_PETRO(COMPRESS=YES KEEP=ArquivoXML ANO GRANDE_AREA AREA SEXO TOT_COAUTOR BANCA NOTA_CAPES 
                            PETRO_LAB PETRO_LATTES PETRO_PETRO UF_END_PROFISSIONAL UF_NASCIMENTO CONTINENTE 
                            LOCAL_FORMAC IDADE_ACADEMICA VINCULO ORIENTA CURSO APRESEN PROD_TEC);
SET LATTES.TABELA_DINAMICAZ(WHERE=(1990<=ANO<=2022 AND 0<IDADE_ACADEMICA<100));

WHERE GRANDE_AREA IN ('ENGENHARIAS','CIENCIAS_EXATAS_E_DA_TERRA','CIENCIAS_BIOLOGICAS')
      AND CONTINENTE NOT IN ('Asia');
...
ARTIGO   = SUM(ARTIGO_BRASIL,ARTIGO_EXTERIOR,CAP_LIVRO_BRASIL,CAP_LIVRO_EXTERIOR,DESENHO_IND_BRASIL,DESENHO_IND_EXTERIOR,
               PATENTE_BRASIL,PATENTE_EXTERIOR,SOFTWARE_BRASIL,SOFTWARE_EXTERIOR,LIVRO_BRASIL,LIVRO_EXTERIOR);
RUN;
